unit RAB_SmashSplitter_Part1;
    ...
    // Init global variables:
        var
            ...
            // Main function:
                Int_CurrentQL, Int_FilesInThisQL, Int_SigsInThisQL, Int_Loop_Main, Int_ProcessedMasters, Int_Iteration, Int_MastersInThisPlugin: integer;
    ...
    function Quickloader_Finish(): null; begin
        ...
        // Display something to the screen:
            ...
            Int_ProcessedMasters := 0;
            Int_Iteration := 0;
            while Int_ProcessedMasters < Int_FilesInThisQL do begin
                
            // Create the file:
                AddMessage( 'Creating QuickLoader plugin...' );
                File_CurrentQL := FileByName('SmashSplitQL' + IntToStr(Int_CurrentQL) + '.' + IntToStr(Int_Iteration) + '.esp');
                if GetLoadOrder(File_CurrentQL) < 0 then File_CurrentQL := AddNewFileName('SmashSplitQL' + IntToStr(Int_CurrentQL) + '.' + IntToStr(Int_Iteration) + '.esp', True);
                AddMessage( ' ' );
                
            // Set the description:
                AddMessage( 'Setting description...' );
                Entry_Decription := Add(ElementByIndex(File_CurrentQL,0), 'SNAM', false);
                SetEditValue(Entry_Decription, 'SmashSplitQuickLoader:' + Arr_SigsInQL[Int_CurrentQL] + ', part:' + IntToStr(Int_Iteration));
                AddMessage( ' ' );
                
                
            // Add the masters:
                Int_MastersInThisPlugin := Int_FilesInThisQL - Int_ProcessedMasters;
                if Int_MastersInThisPlugin > 200 then
                    Int_MastersInThisPlugin := 200;
                AddMessage( 'Adding ' + IntToStr(Int_MastersInThisPlugin) + ' files as masters...' );
                ...
                for Int_LoopMasters := Int_ProcessedMasters to Int_MastersInThisPlugin + Int_ProcessedMasters do begin
                    if Arr_AddAllMasters[Int_LoopMasters] <> '' then
                        AddMasterIfMissing(File_CurrentQL,Arr_AddAllMasters[Int_LoopMasters]);
                end;
                Int_ProcessedMasters := Int_ProcessedMasters + Int_MastersInThisPlugin;
                AddMessage( ' ' );
                
            // Increase plugin iteration
